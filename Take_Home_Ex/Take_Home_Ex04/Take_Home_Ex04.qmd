...
```{r}
pacman::p_load(ggplot2, plotly, ggridges, gganimate, sunburstR, d3r, CGPfunctions, ggHoriPlot, streamgraph, timetk, lubridate, readxl, tidyverse)
```
...
merchTotalMills %>%
  ...
  sg_axis_x(Date, tick_interval = 3, tick_format = " %b %y")
```
...
sund2b(data = treeRegion, valueField = "totalAmount")
```
...
sunburst(data = treeCountry, valueField = "totalAmount", legend = FALSE)
```
